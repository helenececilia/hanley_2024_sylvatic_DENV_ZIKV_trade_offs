...
# instead of having 2 controls in each squirrel monkey dataset
# have the 4 controls in each of the dataset
df2 <- rbind(df2,cont3)
df3 <- rbind(df3,cont2)
...
# for axis limit
# max(c(df1$temp,df2$temp,df3$temp)) # 40.65
...
pt <- pt1 | pt2 | pt3
pt <- pt + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 23))
...
pw <- pw1 | pw2 | pw3
pw <- pw + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 23))
